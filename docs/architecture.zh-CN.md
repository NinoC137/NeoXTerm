...
NeoXTerm 有意将稳定的操作模型放在 Rust 核心中，再通过 CLI 和两个交互式客户端开放出来。这样桌面端或浏览器 UI 都不会成为设备行为的事实来源。

```mermaid
flowchart LR
  User["CLI、浏览器 UI 或桌面应用"] --> Core["NeoXTerm Rust 模块"]
...
ADB 每次操作前都会解析所选设备。NeoXTerm 可以把不随 USB 端口变化的身份锚定为独立信息，再在已知设备换 USB 口时重新选择在线 serial。网络 ADB 与 `usb:` 选择器会被明确区分，而不是仅根据冒号是否存在来判断。
...
`nxt ui` 默认只在 loopback 上提供本地浏览器 UI。它的 PTY 桥是独立 WebSocket 端点。服务端在 PTY 写端就绪前不会放行启动输入；客户端也会在收到明确的 ready 消息前排队输入。这一点很重要：WebSocket 握手完成并不代表子 PTY 已经能接收字节。

### Tauri 桌面工作台

原生客户端位于 `desktop/`，使用 Tauri、React 和 xterm.js 构建。它直接引入 NeoXTerm Rust crate，而不是复制一套通道逻辑。它的终端端点与浏览器工作台相互独立，但遵循同样的就绪和有序输入规则。客户端提供设备总览、发现、档案、终端、操作和插件视图；可能造成影响的向导型工作流会先生成计划，随后才允许显式执行。
