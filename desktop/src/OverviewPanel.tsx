...
export function OverviewPanel({ devices, busy, onRefresh, onSelect }: Props) {
  ...
  return <section className="overview-panel">
    <header className="overview-header"><div><strong>NeoXTerm Fleet Overview</strong><span>Parallel reachability checks and saved device fingerprints</span></div><button className="command-button" onClick={onRefresh} disabled={busy}><RefreshCcw size={15} className={busy ? "spin" : ""} />Refresh fleet</button></header>
